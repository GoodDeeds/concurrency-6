...
#ifdef _WIN32
	Bengine::fatalErrorSocket(message + std::string(" | sys error: ") + std::to_string(WSAGetLastError()));
#else // !_WIN32
...
void socketClient::receiveBytes(char *buffer)
{
	...
	{
		Bengine::fatalErrorSocket("Failed to send! (NET_SOCKET_ERROR)");
	}
...
void socketServer::select_activity()
{
	...
	SOCKET s, new_socket;
	std::string msg1 = "Connected to Server \0 \n";

	char *message = new char[msg1.length() + 1];
